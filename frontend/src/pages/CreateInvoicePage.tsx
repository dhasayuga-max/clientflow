...
import { invoiceApi, clientApi, settingsApi } from '../api';
...
export default function CreateInvoicePage() {
  ...
  const { register, handleSubmit, control, watch, setValue, reset, formState: { errors } } = useForm<InvoiceFormData>({
    defaultValues: {
      services: [{ serviceName: '', serviceDescription: '', hsnSac: '', quantity: 1, unit: 'NOS', price: 0, total: 0 }],
      taxRate: 0,
      paidAmount: 0,
      paymentMethod: 'Cash',
      bankDetails: { bankName: '', accountNumber: '', branchIfsc: '' },
  ...
  // Pre-fill bank details from Settings default when creating a new invoice
  useEffect(() => {
    if (!isEdit && settingsData?.bankDetails) {
      const bd = settingsData.bankDetails;
      if (bd.bankName || bd.accountNumber || bd.branchIfsc) {
        setValue('bankDetails', {
          bankName: bd.bankName || '',
          accountNumber: bd.accountNumber || '',
          branchIfsc: bd.branchIfsc || '',
        });
      }
    }
  }, [settingsData, isEdit, setValue]);
    ...
    if (existingInvoice) {
      reset({
        ...
        bankDetails: {
          ...
          branchIfsc: existingInvoice.bankDetails?.branchIfsc || '',
        },
  ...
  const onSubmit = async (data: InvoiceFormData) => {
    ...
    try {
      const payload = {
        ...data,
        services: data.services.map(s => ({ ...s, total: Number(s.quantity) * Number(s.price), quantity: Number(s.quantity), price: Number(s.price) })),
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="card p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input className="input" {...register('reference')} placeholder="e.g. PO number, ref ID" />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="number" className="input" min="0" step="0.01" {...register('paidAmount')} placeholder="0" />
            </div>
          </div>
        </div>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button type="button" onClick={() => append({ serviceName: '', serviceDescription: '', hsnSac: '', quantity: 1, unit: 'NOS', price: 0, total: 0 })} className="btn-secondary text-xs py-1.5">
          ...
          <div className="space-y-3">
            {fields.map((field, index) => (
              <div key={field.id} className="grid grid-cols-12 gap-2 items-start pb-3 border-b border-gray-100 dark:border-gray-800 last:border-0">
                ...
                <div className="col-span-6 sm:col-span-2">
                  {index === 0 && <label className="label text-xs">HSN/SAC</label>}
                  <input className="input" placeholder="998314" {...register(`services.${index}.hsnSac`)} />
                </div>
                ...
                <div className="col-span-3 sm:col-span-2">
                  {index === 0 && <label className="label text-xs">Unit</label>}
                  <input className="input" placeholder="NOS" {...register(`services.${index}.unit`)} />
                </div>
        ...
        {/* Bank Details */}
        <div className="card p-5">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-1">Bank Details (Optional)</h3>
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">Pre-filled from your default Settings — edit here if this invoice needs different details.</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="label">Bank Name</label>
              <input className="input" {...register('bankDetails.bankName')} placeholder="HDFC Bank" />
            </div>
            <div>
              <label className="label">Account Number</label>
              <input className="input" {...register('bankDetails.accountNumber')} placeholder="50100123456789" />
            </div>
            <div>
              <label className="label">Branch & IFSC</label>
              <input className="input" {...register('bankDetails.branchIfsc')} placeholder="Coimbatore / HDFC0001234" />
            </div>
          </div>
        </div>
        ...
        <div className="card p-5">
          ...
          <textarea className="input" rows={3} {...register('notes')} placeholder="Payment terms or any special instructions..." />
        </div>
