        ...
        <div className="bg-primary-600 p-6 text-white">
          <div className="flex justify-between items-start">
            <div>
              ...
              {invoice.reference && <p className="text-primary-200 text-xs mt-1">Ref: {invoice.reference}</p>}
            </div>
            <div className="text-right">
              ...
              {invoice.paymentMethod && (
                <>
                  <p className="text-primary-200 text-xs mt-1">Payment</p>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              {invoice.state && <p className="text-gray-500 dark:text-gray-500 text-sm">State: {invoice.state}</p>}
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-800">
                  ...
                  <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-3 py-2">HSN/SAC</th>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {invoice.services.map((s: { serviceName: string; serviceDescription?: string; hsnSac?: string; quantity: number; unit?: string; price: number; total: number }, i: number) => (
                  <tr key={i}>
                    ...
                    <td className="px-3 py-3 text-sm text-gray-500 dark:text-gray-400">{s.hsnSac || '—'}</td>
                    <td className="px-3 py-3 text-sm text-right text-gray-700 dark:text-gray-300">{s.quantity} {s.unit || 'NOS'}</td>
          ...
          <div className="flex justify-end">
            <div className="w-full sm:w-64 space-y-2">
              ...
              <div className="flex justify-between text-sm text-emerald-600 dark:text-emerald-400 font-medium">
                <span>Paid Amount</span><span>{formatCurrency(invoice.paidAmount || 0)}</span>
              </div>
              {(invoice.totalAmount - (invoice.paidAmount || 0)) > 0 && (
                <div className="flex justify-between text-sm text-red-600 dark:text-red-400 font-medium">
                  <span>Balance Due</span><span>{formatCurrency(invoice.totalAmount - (invoice.paidAmount || 0))}</span>
                </div>
              )}
          ...
          {invoice.bankDetails && (invoice.bankDetails.bankName || invoice.bankDetails.accountNumber) && (
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm">
                <p className="text-gray-600 dark:text-gray-400"><span className="text-gray-400">Bank:</span> {invoice.bankDetails.bankName || '—'}</p>
                <p className="text-gray-600 dark:text-gray-400"><span className="text-gray-400">A/c No:</span> {invoice.bankDetails.accountNumber || '—'}</p>
                <p className="text-gray-600 dark:text-gray-400"><span className="text-gray-400">IFSC:</span> {invoice.bankDetails.branchIfsc || '—'}</p>
              </div>
            </div>
          )}
