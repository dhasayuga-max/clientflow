...
import {
  ...
  Bold, Italic, FileText, Presentation
} from 'lucide-react';
...
// Populate a fabric canvas from a SlideDefinition
function populateCanvas(canvas: fabric.Canvas, slide: SlideDefinition, interactive: boolean) {
  canvas.clear();
...
function extractSlide(canvas: fabric.Canvas, id: string, name: string): SlideDefinition {
  const json = canvas.toJSON(['selectable','hasControls','editable','rx','ry','charSpacing','lineHeight','x1','y1','x2','y2']) as unknown as { objects?: Array<Record<string, unknown>> };
  return {
    id, name,
  ...
  const handlePDF = async () => {
    ...
    try {
      ...
      for (let i = 0; i < slideData.current.length; i++) {
        ...
        populateCanvas(tc, slideData.current[i], false);
  ...
  return (
    <div className="flex flex-col bg-gray-100 dark:bg-gray-950" style={{ height: '100vh', overflow: 'hidden' }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Canvas */}
        <div className="flex-1 overflow-auto bg-gray-300 dark:bg-gray-800 flex items-start justify-center p-8">
      ...
      <div className="h-7 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 flex items-center px-4 text-xs text-gray-400 gap-3 flex-shrink-0">
        ...
        <span>·</span>
        <span>Click to select · Double-click text to edit · Drag to move</span>
      </div>
    </div>
  );
}
