...
import { Building2, Mail, MessageCircle, Save, Loader2, Upload, Eye, EyeOff, Landmark } from 'lucide-react';
import { settingsApi } from '../api';
import { resolveFileUrl } from '../utils';

type Tab = 'company' | 'bankDetails' | 'email' | 'whatsapp';
...
export default function SettingsPage() {
  ...
  const bankForm = useForm({ defaultValues: { bankName: '', accountNumber: '', branchIfsc: '' } });
  ...
  useEffect(() => {
    if (settingsData) {
      ...
      if (settingsData.bankDetails) bankForm.reset(settingsData.bankDetails);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex gap-2 p-1 bg-gray-100 dark:bg-gray-800 rounded-xl w-fit flex-wrap">
        ...
        <TabButton active={activeTab === 'bankDetails'} onClick={() => setActiveTab('bankDetails')}>
      ...
      {activeTab === 'bankDetails' && (
        <div className="card p-6">
          <div className="mb-5">
            <h3 className="font-semibold text-gray-900 dark:text-white">Default Bank Details</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              This account auto-fills on every new invoice. You can still override it per-invoice if a specific client needs different details.
            </p>
          </div>
          <form onSubmit={bankForm.handleSubmit(data => saveSection('bankDetails', data))} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className="label">Bank Name</label>
                <input className="input" {...bankForm.register('bankName')} placeholder="HDFC Bank" />
              </div>
              <div>
                <label className="label">Account Number</label>
                <input className="input" {...bankForm.register('accountNumber')} placeholder="50100123456789" />
              </div>
              <div>
                <label className="label">Branch & IFSC</label>
                <input className="input" {...bankForm.register('branchIfsc')} placeholder="Coimbatore / HDFC0001234" />
              </div>
            </div>
            <div className="flex justify-end pt-2">
              <button type="submit" disabled={isSaving} className="btn-primary">
