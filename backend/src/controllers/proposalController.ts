...
import { generateEditorPPTX } from '../services/editorPptxService';
...
export async function downloadProposalPPTX(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    // If the proposal has been edited in the canvas editor, export exactly
    // what's on the canvas (text edits, moved elements, images, etc).
    // Otherwise fall back to the original structured-fields template generator.
    let buf: Buffer;
    if (proposal.editorData) {
      buf = await generateEditorPPTX(proposal);
    } else {
      const settings = await Settings.findOne({ userId: req.user!.id });
      buf = await generateProposalPPTX(proposal, settings);
    }
