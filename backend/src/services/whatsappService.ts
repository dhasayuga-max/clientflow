...
export async function sendWhatsApp(
  ...
  if (type === 'invoice' || type === 'reminder') {
    ...
    toNumber = settings.whatsapp.senderNumber; // fallback — proposal no longer stores client phone
    message = `Hello ${proposal.clientName},\n\nPlease review our Business Proposal #${proposal.proposalNumber} for ${proposal.companyName}.\n\nWe look forward to working with you!\n\n${settings.company?.name || ''}\n${settings.company?.website || ''}`;
  }
