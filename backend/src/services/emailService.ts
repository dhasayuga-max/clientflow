...
export async function sendEmail(
  ...
  if (type === 'invoice' || type === 'reminder') {
    ...
    // For proposals, we send to company name — no email field on new schema
    // The caller should ensure email is passed via settings or handled upstream
    toEmail = settings.company?.email || smtpUser;
    filename = `${proposal.proposalNumber}.pdf`;
    subject = `Business Proposal #${proposal.proposalNumber} from ${settings.company?.name || 'Us'}`;
    htmlBody = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #E8762C;">Business Proposal</h2>
        <p>Hello <strong>${proposal.clientName}</strong>,</p>
        <p>Please find attached our business proposal <strong>#${proposal.proposalNumber}</strong> for ${proposal.companyName}.</p>
        <p>We look forward to working with you!</p>
        <hr style="border-color: #E5E7EB;"/>
        <p style="color: #6B7280; font-size: 12px;">${settings.company?.name || ''} &mdash; ${settings.company?.website || ''}</p>
      </div>`;
  }
